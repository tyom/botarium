...
/**
 * Read bot configuration from bots.yaml
 * Returns array of { name, source, entry } objects
 */
function readBotsConfig() {
  const yamlPath = app.isPackaged
    ? path.join(process.resourcesPath, 'bots.yaml')
    : path.join(__dirname, 'bots.yaml')

  if (!fs.existsSync(yamlPath)) {
    electronLogger.info('No bots.yaml found - running in discovery mode')
    return []
  }
  ...
  try {
    ...
    for (const line of yaml.split('\n')) {
      ...
      if (trimmed.startsWith('- name:')) {
        if (currentBot) bots.push(currentBot)
    ...
    return bots.filter((b) => b.name && b.source)
  } catch (err) {
    electronLogger.error({ err }, 'Failed to read bots.yaml')
    return []
  }
}
 ...
 * In dev mode: returns configs to run from source
 * In production/bundled mode: returns configs for compiled binaries
 */
