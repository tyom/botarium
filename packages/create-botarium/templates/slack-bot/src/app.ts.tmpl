...
import { slackConfig, config } from './runtime-config'
{{#if isObservability}}
...
// Config server (set when server starts in simulator mode)
let configServer: ReturnType<typeof createConfigServer>
...
async function waitForWebSocketConnection(
  ...
  // Capture initial connected_bots count before this bot connects
  ...
  while (Date.now() - startTime < timeoutMs) {
    try {
      ...
      if (response.ok) {
        ...
        // On first successful fetch, capture the initial count
        if (initialConnectedBots === null) {
          initialConnectedBots = currentCount
        }

        // Wait until connected_bots increases beyond the initial count
...
{{#if isAi}}
      ...
      if (data.settings) {
        ...
        for (const key of settingsToApply) {
          const valueFromEmulator = data.settings[key]
          ...
          if (valueFromEmulator === undefined) {
            if (previouslyInjected.has(key)) {
              ...
              slackLogger.debug(`Removed ${key} (no longer in simulator settings)`)
            }
            continue
          }
...
async function main() {
  ...
  if (isSimulatorMode) {
    configServer = createConfigServer({
      config,
      {{#if isObservability}}getHealthResponse,{{/if}}
      logger: appLogger,
    })
    if (configServer) {
  ...
  if (isSimulatorMode && wsClient?.on) {
