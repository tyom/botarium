...
export function getModel() {
  ...
  const apiKeyMap = {
    ...
    openrouter: { key: settings.OPENROUTER_API_KEY, name: 'OPENROUTER_API_KEY' },
  } satisfies Record<string, { key: string | undefined; name: string }>

  const apiKeyInfo =
    apiKeyMap[provider as keyof typeof apiKeyMap] ?? apiKeyMap.openai
  if (!apiKeyInfo.key) {
    throw new Error(`Missing ${apiKeyInfo.name} for provider "${provider}". Set ${apiKeyInfo.name} in your environment.`)
  }
...
export function getFallbackModel() {
  ...
  if (!modelId) {
    throw new Error(`No fallback model configured for provider "${provider}". Set MODEL_FAST in your environment.`)
  }
