...
const delegateToExample = tool({
  description: 'Delegate to the example specialist for general queries and demonstrations.',
  ...
  execute: async ({ request }) => {
    const agent = createAgent({
      tools: { exampleTool },
      instructions: 'You are a helpful specialist. Use the exampleTool when appropriate, or answer directly.',
    })
    const result = await agent.generate({ messages: [{ role: 'user', content: request }] })
